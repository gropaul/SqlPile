...
def download_dataset(
        ...
        meta_con: duckdb.DuckDBPyConnection,
        n_rows: int = 1000):
...
meta_con = duckdb.connect('kaggle_datasets.duckdb')


meta_con.create_function("clean_name", clean_name)
meta_con.create_function("table_name_from_file", table_name_from_file)
meta_con.create_function("get_database_file_name_root", get_database_file_name_root)

# attach the kaggle_data database
meta_con.execute("ATTACH 'kaggle_data.duckdb' AS kaggle_data")
...
dataset_con = duckdb.connect('kaggle_data.duckdb')

for ref, schema_name, files in datasets:
    print(f"Downloading {ref} with {len(files)} files")
    download_dataset(ref, schema_name,  files, dataset_con, meta_con, n_rows=100_000)
