...
Categorical columns have the highest duplicate ratios, with a median of 7 duplicates. 
Unlike the other groups, categorical columns show similar duplicate counts for the top 10 and top 100 values. 
This is because they have on average only 39 distinct values, meaning that the top 10
...
\end{subfigure}
  ...
  \caption{String properties per semantic type for columns with at least 10,000 rows (n=548).}
\end{figure*}
...
with 10.0\% having only one character, representing flags or codes. 

Identifier columns are longer, with a average length of 16.6 characters.
However, the standard deviation of lengths for Identifiers is relatively low, with 
31.4\% of columns having all identifier values of the same length. 
The reason for this is mainly uuid identifiers with the top three most common fixed-lengths being 32 (50\%), 36 (19\%) and 24 (12\%). 
