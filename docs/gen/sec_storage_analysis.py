...
with a shorter code. Next, we evaluate FSST~\cite{{boncz_fsst_2020}}, a specialized string compression algorithm that identifies repeated 
substrings, stores them in a dictionary, and replaces them with one-byte codes. We also include FSST12, a 
variant of FSST that uses 12-bit codes to support a larger symbol table. Additionally, we evaluate OnPair~\cite{{gargiulo_onpair_2025}}, 
...
Finally, we benchmark LZ4~\cite{{noauthor_lz4lz4_2025}}, a general-purpose compression algorithm.
...
\insight{{In normalized schemas text columns are frequent but account for a small share of values, as textual attributes are concentrated in smaller dimension tables. 
